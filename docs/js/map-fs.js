...
function ntaStyleOutline() {
  return { fillColor: '#000000', fillOpacity: 0, color: '#ffffff', weight: 1.5, opacity: 0.8, pane: 'ntaPane' };
}

function ntaStyleColored(grade) {
  return { fillColor: GRADE_COLORS[grade] || '#d7263d', fillOpacity: 0.5, color: '#ffffff', weight: 1.5, opacity: 0.8, pane: 'ntaPane' };
}
...
function updateNtaStyle() {
  ...
  if (hexVisible) {
    map.getPane('ntaPane').style.pointerEvents = 'none';
    ntaGeoJSON.eachLayer(function(layer) { layer.setStyle(ntaStyleOutline()); });
  } else {
    map.getPane('ntaPane').style.pointerEvents = 'auto';
    ntaGeoJSON.eachLayer(function(layer) { layer.setStyle(ntaStyleColored(layer.feature.properties.grade)); });
  }
}
...
// -----------------------------------------------------------------------------
// Data loading — walk only on startup, subway lazy
// -----------------------------------------------------------------------------
...
map.on('dragstart', function() { resultCard.classList.remove('open'); searchDropdown.classList.remove('active'); });
